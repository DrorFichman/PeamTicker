...
/**
 * Marker view for the team collaboration chart.
 * Shows player name and stats when a data point is tapped.
 */
public class TeamCollaborationMarkerView extends MarkerView {

    private final TextView playerNameView;
    private final TextView statsView;
    private final List<PlayerTeamCollaborationChartFragment.TeamCollaborationEntry> entries;
    private MPPointF offset;

    public TeamCollaborationMarkerView(Context context, 
            List<PlayerTeamCollaborationChartFragment.TeamCollaborationEntry> entries) {
    ...
    public void refreshContent(Entry e, Highlight highlight) {
        ...
        if (index >= 0 && index < entries.size()) {
            PlayerTeamCollaborationChartFragment.TeamCollaborationEntry entry = entries.get(index);
