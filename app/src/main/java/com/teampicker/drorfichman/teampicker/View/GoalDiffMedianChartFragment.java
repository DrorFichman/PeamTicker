...
public class GoalDiffMedianChartFragment extends Fragment {
    ...
    private static final int LINE_AVG_10 = 0;
    private static final int LINE_AVG_50 = 1;
    private static final int LINE_MEDIAN_50 = 2;
    ...
    private void updateChart() {
        ...
        LineDataSet avg10Set = createMovingDataSet(WINDOW_SIZE_SMALL, false);
        if (avg10Set != null) {
        ...
        if (games.size() >= WINDOW_SIZE_LARGE) {
            ...
            LineDataSet avg50Set = createMovingDataSet(WINDOW_SIZE_LARGE, false);
            if (avg50Set != null) {
     ...
     * @param useMedian If true, calculate median; if false, calculate average
     */
    private LineDataSet createMovingDataSet(int windowSize, boolean useMedian) {
        ArrayList<Entry> entries = new ArrayList<>();

        // Start from windowSize to have enough data
        for (int i = windowSize - 1; i < games.size(); i++) {
            float value = useMedian 
                    ? calculateMedianDiff(i - windowSize + 1, i + 1)
                    : calculateAverageDiff(i - windowSize + 1, i + 1);
    ...
    /**
     * Calculate median goal difference for games in range [startIndex, endIndex)
     */
    private float calculateMedianDiff(int startIndex, int endIndex) {
        int[] diffs = new int[endIndex - startIndex];
        int count = 0;

        for (int i = startIndex; i < endIndex && i < games.size(); i++) {
            Game game = games.get(i);
            diffs[count++] = Math.abs(game.team1Score - game.team2Score);
        }
        ...
        // Sort to find median
        Arrays.sort(diffs, 0, count);

        if (count % 2 == 0) {
            return (diffs[count / 2 - 1] + diffs[count / 2]) / 2f;
        } else {
